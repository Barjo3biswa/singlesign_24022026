...
<div class="col-lg-12">
   <div class="card-header bg-info text-white text-center">
      RESET PASSWORD (for expired only) 
   </div>
   ...
   <div class="card-body" style="margin-top:-35px;">
    <form class="border_login status_form" id='password_expired_form' method="POST">      
        <div class="row">
        ...
        <div class="row mt-3">
            ...
            <div class="col-lg-6">
                ...
                id="pass_change_submit_expired_expired" onclick="passChangeHandleForExpired()">
            </div>
        </div>                                      
    </form>
   </div>
...
<script>
    ...
    //         return;
    //     }
    //     var formData = $('#password_expired_form').serialize();
    //     $.ajax({
    ...
    //             $('#pass_change_submit_expired').show();
    //             $('#otp_div').hide();
    ...
    function passChangeHandleForExpired(){
        event.preventDefault();
        $('#loader_button').show();
        var new_password = $('#new_password').val();
        var result = validateText(new_password);
        ...
        var formData = $('#password_expired_form').serialize();
        $('#pass_change_submit_expired').hide();        
        $.ajax({
            type        : 'POST', // define the type of HTTP verb we want to use (POST for our form)
            url         : 'password_change_process_for_expired.php', // the url where we want to POST
            data        : formData, // our data object
            dataType    : 'json', // what type of data do we expect back from the server
            encode      : true,
            beforeSend: function(){
